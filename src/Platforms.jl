...
export AnyPlatform, CrossPlatform
...
const AnyOrPlat = Union{AnyPlatform,Platform}
"""
...
struct CrossPlatform <: AbstractPlatform
    ...
    encoded::Platform
    function CrossPlatform(host::AnyOrPlat, target::AnyOrPlat)
        return new(host, target, encode_cross_platform(host, target))
    end
end
CrossPlatform(::AnyPlatform, ::AnyPlatform) = AnyPlatform()
CrossPlatform(pair::Pair{<:AnyOrPlat,<:AnyOrPlat}) = CrossPlatform(pair[1], pair[2])
