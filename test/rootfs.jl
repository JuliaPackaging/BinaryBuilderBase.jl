...
@testset "Expand platforms" begin
    ...
    @test expand_microarchitectures(Platform("x86_64", "linux"); cuda="10.1") == [
    ...
    @test expand_microarchitectures(filter!(p -> Sys.islinux(p) && libc(p) == "glibc", supported_platforms())) == [
        ...
        Platform("aarch64", "linux"; libc="glibc", march="armv8"),
        ...
        Platform("powerpc64le", "linux"; libc="glibc", march="power9"),
    ]
...
@testset "Compiler Shards" begin
    ...
    @testset "GCC ABI matching" begin
        ...
        p = Platform("x86_64", "linux"; cxxstring_abi="cxx11")
        @test gcc_version(p, available_gcc_builds) == [v"5.2.0", v"6.1.0"]
        p["march"] = "avx512"
        ...
        @test gcc_version(p, available_gcc_builds) == [v"8.1.0", v"9.1.0", v"11.0.0-iains"]
    end
